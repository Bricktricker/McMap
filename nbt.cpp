	...
	if (_type == tagCompound) {
		...
		if (posVal != dataHolder._compound->end()) {
			if (posVal->second->getType() == tagString) {
				data = std::string(dataHolder._string._data, dataHolder._string._len);
				return true;
			}
		}
	}
